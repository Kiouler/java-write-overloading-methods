...
`git clone https://github.com/ByteLegendQuest/java-write-overloading-methods`

A project directory named `java-write-overloading-methods` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-write-overloading-methods` directory which is created just now.
